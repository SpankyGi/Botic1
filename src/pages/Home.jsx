...
function ReserveHeading({ children }) {
  return (
    <RevealText
      as="h2"
      className="font-serif font-light text-4xl md:text-5xl lg:text-6xl text-botic-cream leading-tight tracking-tight mt-4 mb-6 reserve-title"
      threshold={0.35}
      >
      {children}
    </RevealText>
...
function MagneticCta({ to, children }) {
  const ref = useMagnetic(0.3, 90)
  return (
    <Link ref={ref} to={to} className="magnetic-btn btn-gold">
...
return (
  ...
  {/* ── CTA Reserva ── */}
    ...
    <MagneticCta to={routes.reserves}>{t('home.ctaBtn')}</MagneticCta>
    </div>
    </section>
    </>
    )
